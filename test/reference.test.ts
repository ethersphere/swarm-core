...
// Real bee-js hex/CID pairs (test/unit/cid.spec.ts), used as an independent
// external oracle rather than just a self-consistent round-trip.
describe('Reference CID', () => {
  it('encodes and decodes known manifest CIDs', () => {
    ...
    for (const [hex, cid] of pairs) {
      ...
      expect(reference.toHex()).toBe(hex)
      expect(reference.toCid('manifest')).toBe(cid)
    }
  })

  it('encodes and decodes known feed CIDs', () => {
    const pairs: [string, string][] = [
      [
    ...
    for (const [hex, cid] of pairs) {
      const reference = new Reference(cid)
      expect(reference.toHex()).toBe(hex)
      expect(reference.toCid('feed')).toBe(cid)
    }
  })

  it('isValid distinguishes valid CIDs/hex/lengths from garbage', () => {
    expect(Reference.isValid('bah5acgzaitxyvem2pivg7y3hclkfcich3nagzr5mlqdnu5zr4dfypovrb74q')).toBe(true)
    expect(Reference.isValid('ab'.repeat(32))).toBe(true)
    expect(Reference.isValid('not a reference')).toBe(false)
    expect(Reference.isValid(`bah5${'z'.repeat(10)}`)).toBe(false)
  })
})
